...
- Export ran on Wulver (jobs 1182380 / 1182389 / 1182411; general partition, 14 parallel Ghidra instances, ~11 min for 194 binaries). Local WSL run had hit a Ghidra 11.2.1 bug (`AutoAnalysisManager.getTaskTimesString`: `"000".substring(len)` on a negative task time → post-script never runs); zero occurrences on Wulver, worker retries 3× anyway.
