...
- Optional, separately labelled row **"+ val-tuned abstention"** (`punstrip_abstain_row.py`): one confidence threshold per head tuned on Punstrip-val under their full preset, applied to test — the analogue of BLens's LORD threshold (46% abstention). Never folded into the headline; the fair row has no abstention. No change to the training objective (their objectives are label-vocabulary-aligned by construction; ours stays open-vocabulary seq2seq).
