...
- (a) **Their scorer**: `blens/evaluation/evaluator.py` logic (label-set micro-F1 over XFL canonical tokens, forbidden labels removed, free functions handled as they do), with our raw predictions mapped through `NLP.tristan_canonical_name` (their canonicaliser). Validation: re-scoring their csv columns must reproduce their Table 3 (BLens 0.460 cross-project / 0.294 strict, ±0.005) before our numbers are read.
