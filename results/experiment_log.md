...
## 2026-09-24 01:30 ET — Precision / recall for the LineageBench main table (job 1319532, `scripts/score_pr.py`)
- Common keys 268,136 (ours ∩ SymGen ∩ BLens). Function-level means: routed P 0.249 / R 0.234 / F1 0.237 / EM 10.7%; G 0.226 / 0.209 / 0.213 / 5.9%; R 0.141 / 0.138 / 0.138 / 10.2%; SymGen-34B 0.154 / 0.144 / 0.145 / 2.9%; BLens 0.077 / 0.054 / 0.060 / 1.4%.
- Package-level: routed 0.480 / 0.471 / 0.472; SymGen 0.202 / 0.196 / 0.194; BLens 0.206 / 0.163 / 0.175 (key set differs by 510 rows from the 267,626-key run, hence ±0.004 on baselines' macro F1; paper keeps the earlier macro F1 and adds function-level P/R, which agree with the table's fn F1 within 0.001).
- FT/NCT P/R: routed FT 0.157/0.143, NCT 0.706/0.688; SymGen FT 0.127/0.117, NCT 0.286/0.277; BLens FT 0.023/0.012, NCT 0.348/0.265.
- Report `results/dualhead_v2/score_pr.json` (Wulver `dh2/results/c_lmemb_knn/score_pr.json`). Paper Table 5 now has P (fn) and R (fn) columns.

## 2026-09-24 03:20 ET — Reviewer-round data checks (job 1320029, `scripts/dump_per_pkg.py`; local `scripts/bootstrap_ci.py`)
- **Audit of retrieval EM on novel names:** 48 test functions (all novel-known, 0.02%) have retrieval EM although the name is "novel": their raw symbol is absent from training but the canonical form (demangled, templates stripped) coincides with a training name; the seen flag is computed on raw symbols (0 disagreements with raw_in_train). 199 of 234,657 novel rows have a canonical name in training; reclassifying them changes no reported number by >0.001. Paper Table 6 note rewritten accordingly.
- **Package-level paired bootstrap (10,000 resamples of the 50 packages, common key set):** routed − SymGen +0.278 [0.194, 0.364]; routed − BLens +0.296 [0.241, 0.355]; routed − G +0.072 [0.048, 0.096]; routed − R +0.052 [0.033, 0.072]; G − SymGen +0.206 [0.142, 0.271]. No interval includes 0. Package-level means on this key set: routed 0.472, G 0.400, R 0.419, SymGen 0.194, BLens 0.175.
