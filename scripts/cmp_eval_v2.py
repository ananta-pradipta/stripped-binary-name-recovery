...
tags = sys.argv[1:]
for tag in tags:
    r = json.load(open(f"$WORKSPACE/dh2/results/{tag}_eval_greedy.json"))
...
if len(tags) == 2:
    a = json.load(open(f"$WORKSPACE/dh2/results/{tags[0]}_eval_greedy.json"))["tiers"]["test"]["per_pkg"]
    b = json.load(open(f"$WORKSPACE/dh2/results/{tags[1]}_eval_greedy.json"))["tiers"]["test"]["per_pkg"]
