#!/bin/bash
# Upload preprocessed data and code to HPC HPC
...
echo "=== Uploading to HPC HPC ==="
echo "Remote: ${REMOTE_DIR}"
echo ""

# Create remote directory structure
echo "[1/8] Creating remote directories..."
ssh ${HPC} "mkdir -p ${REMOTE_DIR}/{data/bpe_model,src,configs,checkpoints,scripts,demo,results}"

# --- Large directories: tar + upload + extract ---
mkdir -p ${TMP_DIR}

echo "[2/8] Packing and uploading data/graphs (2.2GB, 241K files)..."
if ssh ${HPC} "[ -d ${REMOTE_DIR}/data/graphs ] && [ \$(ls ${REMOTE_DIR}/data/graphs/ 2>/dev/null | head -5 | wc -l) -gt 0 ]" 2>/dev/null; then
    # Check file count on remote
    REMOTE_GRAPHS=$(ssh ${HPC} "find ${REMOTE_DIR}/data/graphs -type f 2>/dev/null | wc -l")
    ...
    if [ "${REMOTE_GRAPHS}" -eq "${LOCAL_GRAPHS}" ]; then
        ...
        scp ${TMP_DIR}/graphs.tar.gz ${HPC}:${REMOTE_DIR}/data/
        ssh ${HPC} "cd ${REMOTE_DIR}/data && rm -rf graphs && tar xzf graphs.tar.gz && rm graphs.tar.gz"
        echo "  Done."
    fi
else
    echo "  Compressing..."
    ...
    ssh ${HPC} "cd ${REMOTE_DIR}/data && tar xzf graphs.tar.gz && rm graphs.tar.gz"
    echo "  Done."
fi

echo "[3/8] Packing and uploading data/labels (27MB, 450 files)..."
tar czf ${TMP_DIR}/labels.tar.gz -C ${LOCAL_DIR}/data labels
scp ${TMP_DIR}/labels.tar.gz ${HPC}:${REMOTE_DIR}/data/
ssh ${HPC} "cd ${REMOTE_DIR}/data && rm -rf labels && tar xzf labels.tar.gz && rm labels.tar.gz"
...
scp ${TMP_DIR}/external_calls.tar.gz ${HPC}:${REMOTE_DIR}/data/
ssh ${HPC} "cd ${REMOTE_DIR}/data && rm -rf external_calls && tar xzf external_calls.tar.gz && rm external_calls.tar.gz"
...
scp ${TMP_DIR}/string_refs.tar.gz ${HPC}:${REMOTE_DIR}/data/
ssh ${HPC} "cd ${REMOTE_DIR}/data && rm -rf string_refs && tar xzf string_refs.tar.gz && rm string_refs.tar.gz"

# --- Small files: direct scp ---
echo "[6/8] Uploading index files, vocabs, BPE model..."
scp ${LOCAL_DIR}/data/match_index.json ${HPC}:${REMOTE_DIR}/data/
scp ${LOCAL_DIR}/data/split_assignments.json ${HPC}:${REMOTE_DIR}/data/
scp ${LOCAL_DIR}/data/votes_vocab.json ${HPC}:${REMOTE_DIR}/data/
scp ${LOCAL_DIR}/data/build_manifest.json ${HPC}:${REMOTE_DIR}/data/ 2>/dev/null || true
scp -r ${LOCAL_DIR}/data/bpe_model ${HPC}:${REMOTE_DIR}/data/

echo "[7/8] Uploading source code, configs, scripts, demo, checkpoints..."
scp -r ${LOCAL_DIR}/src ${HPC}:${REMOTE_DIR}/
scp -r ${LOCAL_DIR}/configs ${HPC}:${REMOTE_DIR}/
scp -r ${LOCAL_DIR}/scripts ${HPC}:${REMOTE_DIR}/
scp ${LOCAL_DIR}/checkpoints/pretrained_encoder.pt ${HPC}:${REMOTE_DIR}/checkpoints/

# Demo data
tar czf ${TMP_DIR}/demo.tar.gz -C ${LOCAL_DIR} demo
scp ${TMP_DIR}/demo.tar.gz ${HPC}:${REMOTE_DIR}/
ssh ${HPC} "cd ${REMOTE_DIR} && rm -rf demo && tar xzf demo.tar.gz && rm demo.tar.gz"
...
echo "  bash cs785/scripts/HPC_setup.sh     # first time only"
echo "  cd ${REMOTE_DIR}"
echo "  sbatch scripts/HPC_train.sbatch"
echo "  squeue -u USER"
