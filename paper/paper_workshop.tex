...
\begin{table}[t]
    \centering
    \caption{Main results on the 300K-trained 25M model. XProj-4: original cross-project set (10{,}467 fns). XProj-7: Clean 7 extension adds dash, gettext, psmisc (13{,}581 fns). The adaptive gate routes binaries by external-call Jaccard coverage.}
    \label{tab:main}
    \footnotesize
    \begin{tabular}{lcccccc}
        \toprule
        \textbf{Method} & \textbf{Test EM} & \textbf{Test F1} & \textbf{XPr-4 F1} & \textbf{XPr-4 EM} & \textbf{XPr-7 F1} & \textbf{XPr-7 EM} \\
        \midrule
        Decoder (beam $k{=}5$)          & 70.6\% & 0.770 & 0.668 & 53.9\% & 0.665 & 56.5\% \\
        $k$-NN unfiltered               & 70.8\% & 0.770 & 0.698 & 55.6\% & --- & --- \\
        $k$-NN + BinFilter $\tau{=}0.5$ & 70.8\% & 0.770 & \textbf{0.715} & \textbf{57.0\%} & 0.558 & 44.7\% \\
        Adaptive gate $\tau_b{=}0.7$    & \textbf{71.0\%} & \textbf{0.777} & 0.714 & 56.8\% & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Per-package cross-project results. Upper group: original four-package set (Paper 4). Lower group: three low-coverage additions (Clean 7). $J_{\max}$ is the target binary's max external-call Jaccard to any training binary; the adaptive gate routes $J_{\max}\!\ge\!0.7$ to $k$-NN and the rest to the LM-pretrained decoder.}
    ...
    \begin{tabular}{lrcccc}
        ...
        \midrule
        \multicolumn{6}{l}{\emph{Clean 7 extension}} \\
        dash     & 1{,}324 & 0.29 & \textbf{0.770} & 0.130 & 0.815 \\
        gettext  & 1{,}518 & 0.17 & \textbf{0.807} & 0.036 & 0.834 \\
        psmisc   &    272 & 0.39 & \textbf{0.745} & 0.167 & 0.761 \\
        \midrule
        \textbf{Paper 4 aggregate} & \textbf{10{,}467} & --- & 0.668 & \textbf{0.715} & 0.714 \\
        \textbf{Clean 7 aggregate} & \textbf{13{,}581} & --- & 0.665 & 0.558 & \textbf{0.738} \\
        \bottomrule
    \end{tabular}
\end{table}
