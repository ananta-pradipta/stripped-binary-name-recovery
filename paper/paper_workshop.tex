...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        ...
        SymLM (BLens repro)~\cite{symlm,blens} & CCS 2022 & --- & 0.277 \\
        LLM Zero-Shot (StarCoder-3B)        & --- & 3B & 0.654 \\
        \sysname{} (ours)                   & --- & \textbf{32M} & \textbf{0.718} \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

\sysname{} beats the released SYMGEN checkpoint by +0.27 F1 (34B vs.\ 32M parameters), beats our own SYMGEN+LoRA reproduction fine-tuned on the same 300K training data by +0.055 F1, and beats a StarCoder-3B zero-shot baseline by +0.064 F1. llasm and AsmDepictor could not be run in our setting due to released-weight/vocab mismatches and are therefore cited but not tabulated.
...
We do not adjust LLM baseline scores for suspected leakage; the right baseline for a deployed system is the score it achieves with contamination included. \sysname{}'s encoder, by contrast, is trained from scratch on BAP-IR with no exposure to source code, function names from outside the training set, or pretrained text embeddings, so the cross-project numbers genuinely measure generalization beyond the 73 training packages. A future evaluation on packages absent from any large code corpus (private firmware, recently obfuscated code, post-cutoff projects) would likely widen the gap further. The same caveat applies to BLens and SymLM literature numbers in Table~\ref{tab:comparison} (Punstrip Debian binaries are similarly well-represented in any large code corpus). Recent LLM-for-code papers~\cite{symgen,prorec,beyondclass} acknowledge this issue in similar terms.
...
\begin{enumerate}
    ...
    \item The finding that \textbf{$k$-NN retrieval with a binary-fingerprint filter outperforms autoregressive decoding} on cross-project generalization (+2.4pp EM, +0.045 F1) with zero retraining. BinFilter keeps only retrieval candidates whose parent training binary shares at least $\tau{=}0.5$ Jaccard of external-call vocabulary with the query, eliminating cross-package name contamination. The encoder representation plus this retrieval filter, not the decoder, is the key driver of cross-project performance.
...
Our results demonstrate that binary function name recovery can be effectively solved with a 32M-parameter encoder and a filtered nearest-neighbor retrieval head, matching or exceeding systems based on billion-parameter language models at a fraction of the computational cost and without LLM-pretraining contamination.
