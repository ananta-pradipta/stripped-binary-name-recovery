...
Evaluated on 300{,}013 functions from 77 packages, \sysname{} achieves 0.770 F1 on a held-out test set (13{,}559 functions), \textbf{0.738 adaptive-gate F1 across 7 completely unseen packages} (13{,}581 functions spanning nginx forks and lower-coverage targets dash/gettext/psmisc), and 0.715 F1 on the original cross-project subset (10{,}467 functions) used for the SYMGEN head-to-head, outperforming the released SYMGEN~\cite{symgen} checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction fine-tuned on our full 300K training set (0.66), BLens~\cite{blens} (0.46 reported), and a StarCoder-3B zero-shot LLM baseline (0.65), while using only 25M parameters, over 1{,}000$\times$ smaller than SYMGEN's CodeLlama-34B backbone.
...
\begin{itemize}
    ...
    \item We advance the state of the art in cross-project function name recovery, achieving 0.770 F1 on a held-out test set (13{,}559 functions), \textbf{0.738 adaptive-gate F1 across 7 completely unseen packages} (cross-project (nginx118, angie, tengine, recutils, dash, gettext, psmisc; 13{,}581 functions), and 0.715 F1 on the original cross-project set (10{,}467 functions). On the cross-project set, \sysname{} exceeds the released SYMGEN~\cite{symgen} checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction on our full 300K training data (0.66), BLens~\cite{blens} (0.46 reported), and a StarCoder-3B zero-shot LLM baseline (0.65); on a matched 8{,}062-function subset we lead SYMGEN+LoRA by +0.10 F1 / +26 pp EM while using 1{,}000$\times$ fewer parameters. Our code and dataset are publicly available at \href{https://github.com/ananta-pradipta/stripped-binary-name-recovery}{\textcolor{blue}{https://github.com/\allowbreak{}ananta-pradipta/\allowbreak{}stripped-binary-name-recovery}}.
\end{itemize}
...
\subsubsection{Decoder LM Pretraining and Adaptive Gate}
\label{sec:adaptive-gate}
The GRU decoder is pretrained as an unconditional sub-token language model on a 397K-name corpus (73K from our compiled training set, 329K from the XFL release~\cite{xfl}). This equips the decoder with strong C-naming priors (\texttt{parse\_}, \texttt{msg\_}, \texttt{\_cb}) that the fine-tune-only decoder does not acquire from 300K in-domain pairs alone. At inference, a two-level adaptive gate selects between $k$-NN and the decoder per target binary. Let $J_{\max}(B)$ be the maximum Jaccard similarity of the target binary's external-call fingerprint to any training binary. If $J_{\max}(B) \ge \tau_{\text{bin}}{=}0.7$, route the binary's queries to $k$-NN; otherwise route to the decoder, with a per-query safety valve that promotes $k$-NN when top-1 cosine similarity exceeds $\tau_{\text{sim}}{=}0.95$. The threshold emerges from a bimodal distribution where nginx-family packages cluster at $J_{\max} \in [0.97, 1.00]$ and other cross-project packages cluster below $0.70$, a $0.28$-wide gap robust to the specific value in $[0.70, 0.97]$.
...
\begin{table}[t]
    \centering
    \caption{Ablation study on the earlier 87K-function dataset, where all five models share identical splits and are directly comparable. Headline numbers in Section~\ref{sec:main} use the later 300K dataset with the cross-project cross-project set.}
    \label{tab:ablation}
    \footnotesize
...
\begin{table}[t]
    \centering
    \caption{Main results on the 300K-trained 25M model. Cross-project evaluation: 13{,}581 functions from seven fully-held-out packages (nginx118, angie, tengine, recutils, dash, gettext, psmisc). The adaptive gate routes binaries by external-call Jaccard coverage.}
    \label{tab:main}
    \footnotesize
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        \midrule
        Decoder (beam $k{=}5$)          & 70.6\% & 0.770 & 0.665 & 56.5\% \\
        $k$-NN + BinFilter $\tau{=}0.5$ & 70.8\% & 0.770 & 0.558 & 44.7\% \\
        Adaptive gate $\tau_b{=}0.7$    & \textbf{71.0\%} & \textbf{0.777} & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Per-package cross-project results. Upper group: original original cross-project set (original cross-project set). Lower group: three low-coverage additions (cross-project). $J_{\max}$ is the target binary's max external-call Jaccard to any training binary; the adaptive gate routes $J_{\max}\!\ge\!0.7$ to $k$-NN and the rest to the LM-pretrained decoder.}
    \label{tab:perpackage}
    \footnotesize
    \begin{tabular}{lrcccc}
        \toprule
        \textbf{Package} & \textbf{Funcs} & \boldmath$J_{\max}$ & \textbf{Dec} & \textbf{$k$-NN+P2} & \textbf{Gate} \\
        \midrule
        \multicolumn{6}{l}{\emph{original cross-project set}} \\
        nginx118 & 3{,}470 & 1.00 & 0.773 & \textbf{0.878} & \textbf{0.878} \\
        angie    & 3{,}893 & 0.97 & 0.709 & \textbf{0.819} & \textbf{0.819} \\
        tengine  &    554 & 0.70 & \textbf{0.630} & 0.531 & 0.645 \\
        recutils & 2{,}550 & 0.35 & 0.311 & 0.308 & 0.346 \\
        \midrule
        \multicolumn{6}{l}{\emph{}} \\
        dash     & 1{,}324 & 0.29 & \textbf{0.770} & 0.130 & 0.815 \\
        gettext  & 1{,}518 & 0.17 & \textbf{0.807} & 0.036 & 0.834 \\
        psmisc   &    272 & 0.39 & \textbf{0.745} & 0.167 & 0.761 \\
        \midrule
        \textbf{Cross-project aggregate} & \textbf{10{,}467} & --- & 0.668 & \textbf{0.715} & 0.714 \\
        \textbf{Cross-project aggregate} & \textbf{13{,}581} & --- & 0.665 & 0.558 & \textbf{0.738} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Comparison on our cross-project cross-project set. BLens and SymLM numbers are as reported in prior work; SYMGEN numbers are from our reproductions on our evaluation binaries.}
    \label{tab:comparison}
    \footnotesize
    \begin{tabular}{llcc}
        \toprule
        \textbf{System} & \textbf{Venue} & \textbf{Params} & \textbf{Cross-Project F1} \\
        \midrule
        SYMGEN released ckpt~\cite{symgen} & NDSS 2025 & 34B & 0.450 \\
        SYMGEN + LoRA (our 300K)           & NDSS 2025 & 34B & 0.663 \\
        BLens (retrained on our 278K)~\cite{blens}        & USENIX Sec 2025 & $\sim$200M & 0.453 \\
        SymLM (BLens repro)~\cite{symlm,blens} & CCS 2022 & N/A & 0.277 \\
        LLM Zero-Shot (StarCoder-3B)        & N/A & 3B & 0.654 \\
        \sysname{} (ours)                   & N/A & \textbf{25M} & \textbf{0.718} \\
        \bottomrule
    \end{tabular}
\end{table}

\sysname{} beats the released SYMGEN checkpoint by +0.27 F1 (34B vs.\ 25M parameters), beats an end-to-end SYMGEN+LoRA reproduction fine-tuned on the same 300K training data by +0.055 F1, and beats a StarCoder-3B zero-shot baseline by +0.064 F1. llasm and AsmDepictor could not be run in our setting due to released-weight/vocab mismatches and are therefore cited but not tabulated.
...
We do not correct the reported LLM baseline scores for possible pretraining overlap; we interpret them as practical deployment baselines. \sysname{}'s \emph{encoder} is trained from scratch on BAP-IR with no exposure to source code, function names from outside the training set, or pretrained text embeddings --- the cross-project numbers genuinely measure encoder generalization beyond the 73 training packages. \sysname{}'s \emph{decoder}, separately, is LM-pretrained on 397K function names (73K from our compiled training set, 329K from the XFL release~\cite{xfl}); some cross-project ground-truth names appear verbatim in this corpus (dash 85\%, psmisc 67\%, gettext 18\%). This is LM-pretrain exposure to isolated name strings rather than supervised training on (code, name) pairs; at inference the decoder still requires encoder conditioning on novel code to select the correct name. SymGen's CodeLlama-34B backbone has the same category of exposure at larger scale (2T tokens of general source code including Debian packages). The same caveat applies to BLens and SymLM literature numbers (Punstrip Debian binaries are similarly well-represented in code corpora).

On names absent from our pretrain corpus verbatim, we measure whether the decoder can \emph{compose} them from known sub-tokens. Gettext's 421 novel names have 85\% with all sub-tokens present in the corpus and 95.3\% mean sub-token coverage; psmisc's 28 novel names have 61\% fully covered; dash's 53 novel names (mostly GCC-mangled forms like \texttt{\_\_do\_global\_dtors\_aux}) have only 21\% covered. The compositional regime --- especially for gettext --- indicates that the LM pretrain acts as a sub-token prior rather than a memorization look-up, so the decoder is non-trivially generative rather than merely retrieving strings it has seen.
...
\begin{table}[t]
    ...
    \begin{tabular}{llr}
        ...
        Training & 25M params, 50 epochs, AMP batch 256 & 2h 12m \\
        \midrule
...
$k$-NN inference is approximately 10$\times$ faster than beam-search decoding on the test set. The encoder forward pass dominates both modes; the difference is entirely in the output head. \sysname{}'s 25M-parameter model is over 1{,}000$\times$ smaller than SYMGEN's CodeLlama-34B backbone, making it practical for deployment in resource-constrained environments such as incident response workstations.
...
Earlier ablations on the fine-tune-only decoder showed a strong retrieval advantage: 0 of 1,873 truly-unseen function names were correctly predicted, and every correct prediction matched a training name, typically shared GNU utility functions (\texttt{xmalloc}, \texttt{quotearg\_buffer\_\allowbreak{}restyled}, \texttt{set\_program\_name}). In that regime the decoder's compositional capacity was empirically unused, and retrieval won by constraining outputs to real names and avoiding the autoregressive error cascade.

With LM pretraining on a 397K-name corpus, the decoder acquires sub-token priors that enable a non-trivial compositional regime on packages whose sub-tokens are present in the pretraining corpus (e.g., 85\% of gettext's novel names are composable from known sub-tokens). An adaptive binary-coverage gate exploits this: high-coverage binaries (nginx-family, with binary-level Jaccard $\ge 0.7$) route to $k$-NN for exact-name recovery, while low-coverage binaries (dash, gettext, psmisc) route to the pretrained decoder for sub-token composition. SYMGEN~\cite{symgen} demonstrates that LLMs can generate function names with different tradeoffs, though an end-to-end SYMGEN+LoRA reproduction (F1 0.663 on the full 300K fine-tune) still trails our 25M model by +0.055 F1 overall and +0.100 F1 on the matched head-to-head. Our finding is that for GNN-based encoders, retrieval and a targeted sub-token LM pretrain are complementary drivers of cross-project generalization, with an adaptive routing mechanism selecting between them per binary.
...
\begin{enumerate}
    \item \textbf{Partial compositional capacity}: In the fine-tune-only regime, the model could not compose novel function names (0 of 1{,}873 truly-unseen names recovered). With 397K-name LM pretraining plus the adaptive gate, a non-trivial compositional regime emerges on packages whose sub-tokens are present in the pretraining corpus (gettext composes 85\% of its novel names). However, packages whose sub-tokens are also absent (recutils's \texttt{rset}, \texttt{mset}, \texttt{fex}) remain beyond either head's reach, and truly novel naming conventions (e.g., a custom game engine) will still collapse to the training distribution. The $k$-NN datastore partially mitigates this by making the knowledge base explicit and updatable, but the underlying encoder and decoder still reflect training data biases.
...
We presented \sysname{}, a 25M-parameter graph neural network system for binary function name recovery that reaches 0.770 F1 on a held-out test set (13{,}559 functions), \textbf{0.738 adaptive-gate F1 across 7 completely unseen packages} (cross-project (nginx118, angie, tengine, recutils, dash, gettext, psmisc; 13{,}581 functions), and 0.715 F1 on the original cross-project set (10{,}467 functions). On the cross-project set it beats the released SYMGEN checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction on the same 300K training data (0.66), BLens (0.46 reported), and a StarCoder-3B zero-shot baseline (0.65). On a matched 8{,}062-function subset evaluated by both systems we lead SYMGEN+LoRA by $+0.10$ F1 and $+26$pp EM, while using over 1{,}000$\times$ fewer parameters. Our key contributions are:
