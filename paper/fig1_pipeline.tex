...
\begin{document}
    ...
    \node[input, right=of src] (compile) {Compile\\{\scriptsize O0, O2}};
    ...
    \node[output, right=1.0cm of fuse3, yshift=0.5cm] (decoder) {GRU Decoder\\{\scriptsize beam $k{=}5$}};
    \node[output, right=1.0cm of fuse3, yshift=-0.5cm] (knn) {$k$-NN Retrieval\\{\scriptsize $k{=}1$, cosine}};
    \node[input, right=0.7cm of $(decoder)!0.5!(knn)$] (name) {Predicted\\Name};
    \node[input, right=0.6cm of name] (metrics) {Metrics\\{\scriptsize F1, EM}\\{\scriptsize EdSim, NgSim}};

    \draw[arr] (fuse3.east) -- ++(0.25,0) |- node[above right, lbl, pos=0.3] {$\mathbf{z}$} (decoder.west);
    \draw[arr] (fuse3.east) -- ++(0.25,0) |- (knn.west);
    \draw[arr] (decoder.east) -- ++(0.2,0) |- (name.west);
    \draw[arr] (knn.east) -- ++(0.2,0) |- (name.west);
    \draw[arr] (name) -- (metrics);

    \node[phase, fit=(decoder)(knn)(name)(metrics), inner sep=8pt, label={[font=\small\bfseries]above:Phase 3: Inference \& Evaluation}] (p3) {};
