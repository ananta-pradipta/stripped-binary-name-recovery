...
A single-layer GRU decoder with 1024-dimensional hidden state generates function names as sub-token sequences from the Votes vocabulary of 2,642 tokens (Section~\ref{sec:votes}). At each decoding step $t$, the decoder takes the previous token embedding and the hidden state to produce a distribution over the vocabulary:
...
and return the name $n_{i^*}$ of the nearest neighbor $i^* = \arg\max_i \text{sim}(\mathbf{z}_\text{test}, \mathbf{z}_i)$.
