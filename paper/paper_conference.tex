...
\begin{table}[t]
    \centering
    \caption{Main results. XProj-4: original four-package cross-project set (10{,}467 fns). XProj-7: Clean 7 extension adding dash, gettext, psmisc (13{,}581 fns, fully held out). The adaptive gate (Sec.~\ref{sec:adaptive-gate}) routes each binary to $k$-NN or decoder based on $J_{\max}(B)$.}
    \label{tab:main}
    \footnotesize
    \begin{tabular}{lcccccc}
        \toprule
        \textbf{Method} & \textbf{Test EM} & \textbf{Test F1} & \textbf{XProj-4 F1} & \textbf{XProj-4 EM} & \textbf{XProj-7 F1} & \textbf{XProj-7 EM} \\
        \midrule
        Decoder only (beam $k{=}5$) & 70.6\% & 0.770 & 0.668 & 53.9\% & 0.665 & 56.5\% \\
        $k$-NN unfiltered & 70.8\% & 0.770 & 0.698 & 55.6\% & --- & --- \\
        $k$-NN + BinFilter ($\tau{=}0.5$) & 70.8\% & 0.770 & \textbf{0.715} & \textbf{57.0\%} & 0.558 & 44.7\% \\
        Adaptive gate ($\tau_{\text{bin}}{=}0.7$) & \textbf{71.0\%} & \textbf{0.777} & 0.714 & 56.8\% & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
