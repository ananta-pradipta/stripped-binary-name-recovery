...
Patrick-Evans et al.~\cite{xfl} introduced XFL, framing function name prediction as extreme multi-label classification over a closed tag vocabulary, achieving 82.5\% tag precision on Debian binaries. XFL's lasting contribution is the DEXTER embedding, which subsequent ensemble systems (including BLens) reuse as a feature source. However, its multi-label formulation is fundamentally constrained: recent reproduction by Benoit et al.~\cite{blens} reports that XFL produces empty predictions for 60.7\% of cross-project functions (abstention rate), yielding an aggregate sub-token F1 of only 0.22 on the Punstrip cross-project setting. This illustrates a broader structural limitation we address: closed-vocabulary classifiers cannot generate names absent from their training label space.
...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        ...
        \sysname{} ($k$-NN+P2) & --- & Ours & \textbf{0.650} \\
        \sysname{} (decoder) & --- & Ours & 0.612 \\
...
\textbf{Head-to-head with SYMGEN.} On our four-package cross-project test set, the released SYMGEN checkpoint~\cite{symgen}---a 34-billion-parameter CodeLlama-based model fine-tuned on SymGen's own training corpus---achieves 0.450 F1, while \sysname{} reaches 0.650 with $k$-NN retrieval: an absolute improvement of $+0.20$ F1. This comparison uses a model roughly 1{,}000$\times$ smaller (32M versus 34B parameters) trained from scratch on our 300K corpus. We note that SYMGEN's CodeLlama backbone was pre-trained on approximately 2T tokens of source code, likely including upstream nginx sources that dominate our test set; \sysname{}'s from-scratch training has no such implicit exposure, making the gap particularly striking. Reproducing SYMGEN's fine-tuning pipeline on our training data is in progress at the time of submission; the pretrained-checkpoint comparison already represents the realistic deployment scenario where a practitioner uses the released artifact off the shelf.

\textbf{SymLM's closed-vocabulary limitation.} SymLM~\cite{symlm} uses a 3{,}069-class classification head over a fixed label vocabulary. As reported by BLens~\cite{blens} under a unified cross-project evaluation, 43.8\% of test functions fall outside SymLM's trainable vocabulary and are marked ``not in the dataset,'' structurally bounding its recall regardless of encoder quality. Our generative sub-token decoder and retrieval-over-embeddings paradigm both address this limitation by construction: \sysname{} can emit any composition of its 7{,}004 Votes sub-tokens, and the $k$-NN stage retrieves from the full training-name index. The same limitation applies to XFL~\cite{xfl}, whose multi-label classifier abstains on 60.7\% of cross-project functions in BLens's reproduction.

\textbf{BLens and Epitome.} BLens~\cite{blens} and Epitome~\cite{epitome} are methodologically closest to our work---BLens combines DEXTER, CLAP, and PalmTree ensemble embeddings with a specialized LORD decoder, and Epitome adopts votes-based sub-token tokenization similar to ours. Reproducing BLens end-to-end on our test set was infeasible because its preprocessing pipeline requires IDA Pro (a commercial license) and sudo-privileged installs of PostgreSQL, Ghidra, and the XFL-derived DEXTER feature database. Epitome's implementation is not publicly released. We therefore cite their published numbers on the Punstrip Debian dataset as literature anchors.

\textbf{Reproducibility bit-rot.} We note that reproducing SymLM in 2026 was not feasible even with significant engineering effort: SymLM's reported 0.585 F1 requires a Trex~\cite{trex} encoder checkpoint whose original Google Drive location (linked from the SymLM repository) has been deleted, and the newer Trex release has an incompatible state-dict format relative to SymLM's 2021 fairseq fork. The most reliable SymLM cross-project number currently available in the literature is therefore the BLens reproduction (0.277), which we cite above. This mirror-decay of research artifacts is an increasingly common issue in reverse-engineering benchmarks and motivates our open release of both code and dataset.
