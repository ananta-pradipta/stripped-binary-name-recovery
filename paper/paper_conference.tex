...
\begin{itemize}
    ...
    \item We advance the state of the art in cross-project function name recovery, achieving 0.770 F1 on a held-out test set (13{,}559 functions) and \textbf{0.718 F1} across four completely unseen packages (10{,}467 functions: tengine, angie, nginx118, recutils). This exceeds the released SYMGEN checkpoint (0.45), our own SYMGEN+LoRA reproduction on the same 300K training data (0.66), BLens (0.46 reported), and a StarCoder-3B zero-shot baseline (0.65). On a matched 8{,}062-function subset we lead SYMGEN+LoRA by $+0.10$ F1 / $+26$pp EM while using 1{,}000$\times$ fewer parameters. Self-supervised pretraining with partial embedding transfer contributes $+15.9$ percentage points of cross-project EM. Our code and dataset are publicly available at \href{https://github.com/ananta-pradipta/stripped-binary-name-recovery}{\textcolor{blue}{https://github.com/\allowbreak{}ananta-pradipta/\allowbreak{}stripped-binary-name-recovery}}.
\end{itemize}
...
Table~\ref{tab:main} shows the three output heads converge on the in-distribution test set (all three ${\sim}0.77$ F1), but diverge on cross-project. The binary-fingerprint-filtered $k$-NN pulls ahead by keeping only retrieval candidates whose parent training binary has Jaccard external-call overlap $\geq \tau$ with the query binary. This drops candidates from unrelated codebases (bash, sqlite, and so on) that would otherwise contaminate the neighbor pool, lifting cross-project F1 from 0.673 (decoder) to 0.718 with zero retraining. The filter is most valuable on nginx-family packages where many training binaries share external-call vocabulary; on recutils, whose training pool has no high-Jaccard partners, the filter effectively does nothing.
...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        ...
        \sysname{} (ours) & --- & Ours & \textbf{0.718} \\
        SYMGEN + LoRA (full 300K fine-tune) & NDSS 2025 & Ours (15{,}983 fns)~\S & 0.663 \\
        LLM Zero-Shot (StarCoder-3B) & --- & Ours (12{,}391 fns)~\S & 0.654 \\
...
\begin{itemize}
    \item \textbf{Reported numbers stand as-is.} We do not adjust LLM baseline scores for suspected leakage---the right baseline for a deployed system is the score it actually achieves, contamination included.
    ...
    \item \textbf{Cross-project absolute numbers should be read with this caveat.} A future evaluation that includes packages not in any large-scale code corpus---private firmware, recently obfuscated or proprietary code, or post-cutoff projects---would likely widen the gap between \sysname{} and LLM-based baselines, because the LLM's source-text prior would no longer apply.
