...
Evaluated on 300{,}013 functions from 77 packages, \sysname{} achieves 0.770 F1 on a held-out test set (13{,}559 functions) and, under an adaptive binary-coverage gate, \textbf{0.738 F1} across seven fully-held-out packages (13{,}581 functions) covering both nginx forks and lower-coverage targets (dash, gettext, psmisc). On the original original cross-project set (10{,}467 functions) used for the SYMGEN head-to-head, \sysname{} reaches 0.715 F1; the released SYMGEN checkpoint reaches 0.45 and an end-to-end SYMGEN+LoRA reproduction fine-tuned on the same 300K training data reaches 0.66, leaving \sysname{} ahead of the stronger baseline by $+0.06$ F1 on the original cross-project set, by $+0.108$ F1 on the cross-project set, and by $+0.10$ F1 on an apples-to-apples matched 8{,}062-function subset. \sysname{} uses 25M parameters, over 1{,}000$\times$ smaller than SYMGEN's 34B CodeLlama backbone, and was trained from scratch on BAP-IR without any source-code pretraining, so its generalization is not inflated by the kind of LLM-pretraining contamination that benefits SYMGEN on open-source evaluation packages.
...
\begin{itemize}
    ...
    \item We advance the state of the art in cross-project function name recovery, achieving 0.770 F1 on a held-out test set (13{,}559 functions), \textbf{0.738 adaptive-gate F1 across seven fully-held-out packages} (cross-project (nginx118, angie, tengine, recutils, dash, gettext, psmisc; 13{,}581 functions), and 0.715 F1 on the legacy original cross-project set. On the original cross-project set we exceed the released SYMGEN checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction on the same 300K training data (0.66), BLens (0.46 reported), and a StarCoder-3B zero-shot baseline (0.65); on a matched 8{,}062-function subset we lead SYMGEN+LoRA by $+0.10$ F1 / $+26$pp EM while using 1{,}000$\times$ fewer parameters. Self-supervised pretraining with partial embedding transfer contributes $+0.13$ cross-project F1. Our code and dataset are publicly available at \href{https://github.com/ananta-pradipta/stripped-binary-name-recovery}{\textcolor{blue}{https://github.com/\allowbreak{}ananta-pradipta/\allowbreak{}stripped-binary-name-recovery}}.
\end{itemize}
...
\begin{table}[t]
    ...
    \begin{tabular}{clccccc}
        \toprule
        \textbf{\#} & \textbf{Model} & \textbf{Params} & \textbf{Test F1} & \textbf{Test EM} & \textbf{Cross-Proj.\ F1} & \textbf{Cross-Proj.\ EM} \\
        \midrule
...
\begin{table}[t]
    \centering
    \caption{Main results on the held-out test set and the cross-project evaluation (13{,}581 functions from nginx118, angie, tengine, recutils, dash, gettext, psmisc --- all fully held out). The adaptive gate (Sec.~\ref{sec:adaptive-gate}) routes each binary to $k$-NN or decoder based on $J_{\max}(B)$.}
    \label{tab:main}
    \footnotesize
    \setlength{\tabcolsep}{3pt}
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        \textbf{Method} & \textbf{Test EM} & \textbf{Test F1} & \textbf{Cross-Proj.\ F1} & \textbf{Cross-Proj.\ EM} \\
        \midrule
        Decoder only (beam $k{=}5$) & 70.6\% & 0.770 & 0.665 & 56.5\% \\
        $k$-NN + BinFilter ($\tau{=}0.5$) & 70.8\% & 0.770 & 0.558 & 44.7\% \\
        Adaptive gate ($\tau_{\text{bin}}{=}0.7$) & \textbf{71.0\%} & \textbf{0.777} & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    ...
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Method} & \textbf{Test EM} & \textbf{Test F1} & \textbf{Cross-Project EM} & \textbf{Cross-Project F1} \\
        \midrule
...
\begin{table}[t]
    \centering
    \caption{Per-package cross-project results. Seven fully-held-out packages. $J_{\max}$ is the max external-call Jaccard similarity to any training binary; the adaptive gate routes binaries with $J_{\max}\!\ge\!0.7$ to $k$-NN and the rest to the LM-pretrained decoder.}
    ...
    \begin{tabular}{lrcccc}
        ...
        \midrule
        \multicolumn{6}{l}{\emph{original cross-project set (original cross-project set)}} \\
        ...
        \midrule
        \multicolumn{6}{l}{\emph{ (add three low-coverage packages)}} \\
        dash     & 1{,}324 & 0.29 & 0.130          & \textbf{0.770} & 0.815 \\
        gettext  & 1{,}518 & 0.17 & 0.036          & \textbf{0.807} & 0.834 \\
        psmisc   &    272 & 0.39 & 0.167          & \textbf{0.745} & 0.761 \\
        \midrule
        \textbf{Cross-project aggregate} & \textbf{10{,}467} & --- & \textbf{0.715} & 0.668 & 0.714 \\
        \textbf{Cross-project aggregate} & \textbf{13{,}581} & --- & 0.558 & 0.665 & \textbf{0.738} \\
        The original cross-project set, nginx-family only & 7{,}917 & --- & 0.829 & 0.744 & 0.832 \\
        \bottomrule
    \end{tabular}
\end{table}
...
This stratification shows that accuracy scales with the overlap between a package's function names and the training vocabulary, a property shared by all current function-name-recovery systems. Packages that share naming conventions with the training set (even as distinct forks) achieve high accuracy via $k$-NN retrieval; packages with unique codebases fall back on the decoder's LM-pretrained sub-token prior. On a broader cross-project evaluation that adds dash, gettext, and psmisc (all with binary-level Jaccard similarity to training below $0.45$), the adaptive binary-coverage gate raises aggregate F1 from 0.582 (pure $k$-NN + BinFilter) to 0.738 by routing low-coverage binaries to the pretrained decoder, reducing the pure-recognizer bound that BLens~\cite{blens} and SYMGEN~\cite{symgen} also report.

\subsubsection{Two Success Paths: When Retrieval Wins vs.\ When the Decoder Wins}
\label{sec:two-paths}

Table~\ref{tab:perpackage}'s per-package columns reveal two qualitatively distinct success mechanisms, selected per binary by the adaptive gate:

\textbf{Path A --- Retrieval-driven (nginx family).} nginx118, angie, and tengine all satisfy $J_{\max}(B) \ge 0.70$ because nginx itself is in training and shares connection and request-handler primitives with every nginx fork. Pure $k$-NN recovers the exact name from a near-clone neighbor: nginx118 $k$-NN F1 is 0.878 and the adaptive gate adds nothing (0.878). The decoder column on these rows is strictly lower (0.709--0.773), because many nginx internals (\texttt{ngx\_http\_*}, \texttt{ngx\_event\_*}) never appear in the XFL corpus and the decoder's sub-token prior has nothing to bias it toward them. For this regime, retrieval is both necessary and sufficient.

\textbf{Path B --- Decoder-driven (dash, gettext, psmisc).} These three packages have no structural clone in training, so pure $k$-NN F1 collapses to 0.036--0.167. The gate detects the low $J_{\max}(B)$ ($<0.45$) and routes to the LM-pretrained decoder, which composes the correct names from its 397K-name sub-token prior: dash jumps from $k$-NN 0.130 to gate 0.815 ($+0.685$), gettext 0.036 $\to$ 0.834 ($+0.798$), psmisc 0.167 $\to$ 0.761 ($+0.594$). These gains are entirely decoder-side. A retrieval-only system would plateau around cross-project F1 $\approx 0.558$ because Path B packages have no retrievable neighbors; a decoder-only system would plateau around F1 $\approx 0.665$ because it cannot resolve Path A's out-of-distribution identifiers at the sub-token level. The adaptive gate reaches F1 $= 0.738$ by selecting between them per query --- retrieval and generation are not substitutes but cover disjoint failure modes.

\textbf{Why Path B works when the encoder is OOD --- a Bayes factorization.} A cross-project binary whose package never appeared in training produces a weak but topical encoder embedding $z$: the encoder is trained to map $(\text{CFG}, \text{external calls}, \text{token distribution}) \to \text{name}$, and these structural features recur across packages sharing a paradigm (a dash function with a lexer CFG and \texttt{read}/\texttt{malloc} calls maps near bash's lexer region of latent space, even though dash binaries were not in training). The decoder then generates under $P(\text{name} \mid z) \propto P(z \mid \text{name}) \cdot P(\text{name})$: the first factor is weak but topical, localizing to the correct region of name space; the second is the LM prior from the 397K-name XFL corpus (85\%/18\%/67\% verbatim coverage for dash/gettext/psmisc, 95\% sub-token coverage for novel compositions). The product peaks sharply on the correct name because the factors cover complementary signals --- $z$ selects the paradigm region, $P(\text{name})$ the specific composition within that region. Packages with no paradigm sibling in training fail both: $z$ is genuinely uninformative (F1 $<$ 0.03 across all modes) and the LM prior under-covers the vocabulary. Both factors must be non-trivial for Path B to succeed. LM pretraining modifies decoder weights directly; it may also indirectly reshape the encoder during supervised fine-tuning by redirecting the supervised gradient away from sub-token fluency toward paradigm discrimination, but we do not isolate this second-order effect experimentally.
...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        \toprule
        \textbf{System} & \textbf{Venue} & \textbf{Test Set} & \textbf{Cross-Project F1} \\
        \midrule
        \multicolumn{4}{l}{\emph{Reproduced on our cross-project cross-project (10{,}467 functions)}} \\
        ...
        \midrule
        \multicolumn{4}{l}{\emph{Reproduced on our cross-project (19{,}406 functions)}} \\
        \sysname{} (ours, adaptive gate) & N/A & Ours 9-pkg & \textbf{0.738} \\
        SYMGEN + LoRA (full 300K fine-tune) & NDSS 2025 & Ours 9-pkg (18{,}944 fns)~\S & 0.630 \\
        BLens~\cite{blens} & USENIX Sec 2025 & Ours 9-pkg & 0.453 \\
        \midrule
...
Table~\ref{tab:comparison} reports cross-project F1 across published systems in two blocks. The top block contains systems reproduced on identical test data (our cross-project set of 10{,}467 functions): these rows support direct head-to-head claims. The bottom block cites published numbers from the respective system's own evaluation (different test data, different dedup protocols, sometimes different metric definitions) and is included only as a literature anchor. We do not assert transitive orderings across the two blocks.
...
\begin{table}[t]
    \centering
    \caption{Computation cost comparison across systems evaluated on our cross-project cross-project set. Training times are for the fine-tuning/training stage of each system on the same 300K training data; zero-shot baselines report 0. Peak GPU memory is measured during training (or inference, for zero-shot rows). Inference per function is wall-clock latency amortized over batched evaluation.}
...
\begin{figure}[t]
    ...
    \caption{Accuracy versus parameter count on cross-project function naming (log scale). \sysname{} (25M, 0.738 adaptive-gate F1 on cross-project) occupies the top-left corner: high cross-project F1 with two-to-three orders of magnitude fewer parameters than the LLM-based baselines. Markers in the same color reproduce on our cross-project test set; lighter-colored markers (BLens, SymLM) report numbers from the BLens'25 reproduction on Punstrip and are shown for literature-anchor context.}
    \Description{Scatter plot with parameters (log scale, x-axis) versus cross-project F1 (y-axis). GraphR at 25M / 0.738 sits in the upper-left corner of the plot. SymGen+LoRA at 34B / 0.663 and StarCoder-3B at 3B / 0.654 sit further right with comparable or lower F1.}
    \label{fig:pareto}
\end{figure}
...
\textbf{Expanded cross-project head-to-head.} We additionally ran SYMGEN+LoRA on five more cross-project packages (dash, gettext, psmisc, grep, sed) using the same LoRA adapter and Ghidra extraction pipeline. On cross-project (dropping grep+sed as training-contaminated), SYMGEN+LoRA aggregates to F1=0.6301 (18{,}944 fns) versus \sysname{} with adaptive gate at F1=0.7381 (13{,}581 fns). The widest gap is on the three low-coverage additions (dash/gettext/psmisc), where SYMGEN F1=0.4412 trails \sysname{}'s Gate F1=0.8198 by $+0.38$. The gap is driven by the pretrained decoder's ability to compose plausible sub-token combinations for packages whose names are far from retrieval reach; SYMGEN's 34B CodeLlama backbone, despite broader LM pretraining, does not close this specific gap in our evaluation. SYMGEN retains its advantage only on recutils, consistent with the contamination asymmetry discussed above.
...
\begin{itemize}
    ...
    \item \textbf{Cross-project generalization.} The released SYMGEN checkpoint reaches 0.45 F1 on our cross-project test set; \sysname{} reaches 0.718. Even after we fine-tune SYMGEN's CodeLlama-34B on our own 300K training corpus, it plateaus at 0.66, still below \sysname{}'s 25M parameters. The recognizer-vs-composer distinction is what drives this: LLM baselines win on packages their pretraining saw (recutils) and lose on packages it did not (recent nginx forks), confirming that the gap is not explained by parameter count.
...
We presented \sysname{}, a 25M-parameter graph neural network system for binary function name recovery that reaches 0.770 F1 on a held-out test set (13{,}559 functions), \textbf{0.738 F1 with the adaptive binary-coverage gate across seven fully-held-out packages} (cross-project (nginx118, angie, tengine, recutils, dash, gettext, psmisc; 13{,}581 functions), and 0.715 F1 on the original original cross-project set (10{,}467 functions) used for the SYMGEN head-to-head. On the original cross-project set, \sysname{} beats the released SYMGEN checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction fine-tuned on the same 300K training data (0.66), BLens (0.46 reported), and a StarCoder-3B zero-shot baseline (0.65). On a matched 8{,}062-function subset evaluated by both systems, the lead over SYMGEN+LoRA widens to $+0.10$ F1 and $+26$ percentage points of exact match. \sysname{} is over 1{,}000$\times$ smaller than SYMGEN's CodeLlama-34B backbone and was trained from scratch on BAP-IR without any source-code pretraining. Our key contributions are:
