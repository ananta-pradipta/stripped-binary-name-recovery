...
\begin{table}[t]
    \centering
    \caption{Main results: XPr-4 = original 4-pkg set; XPr-7 = Clean 7 extension adds dash/gettext/psmisc. Adaptive gate routes binaries by external-call Jaccard.}
    \label{tab:main}
    \footnotesize
    \begin{tabular}{lcccccc}
        ...
        $k$-NN+P2 & 70.8\% & 0.770 & \textbf{0.715} & \textbf{57.0\%} & 0.558 & 44.7\% \\
        Adaptive gate & \textbf{71.0\%} & \textbf{0.777} & 0.714 & 56.8\% & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Per-package cross-project. Upper: Paper 4. Lower: Clean 7 extension. $J_{\max}$ is max external-call Jaccard to training. Gate routes $J_{\max}\!\ge\!0.7$ to $k$-NN, else decoder.}
    ...
    \begin{tabular}{lrcccc}
        ...
        \midrule
        Paper 4 agg. & \textbf{10{,}467} & --- & 0.668 & \textbf{0.715} & 0.714 \\
        Clean 7 agg. & \textbf{13{,}581} & --- & 0.665 & 0.558 & \textbf{0.738} \\
        \bottomrule
    \end{tabular}
\end{table}
...
We presented \sysname{}, a 25M-parameter GNN+retrieval system that reaches 0.770 F1 on a held-out test set and \textbf{0.718 F1} across 4 completely unseen packages, beating released SYMGEN (0.45), SYMGEN+LoRA (0.66), BLens (0.46), and StarCoder-3B zero-shot (0.65) while being over 1{,}000$\times$ smaller than SYMGEN's backbone. Our contributions:
