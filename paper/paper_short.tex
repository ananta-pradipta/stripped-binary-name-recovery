...
Evaluated on 300{,}013 functions from 77 packages, \sysname{} achieves 0.770 F1 on a held-out test set, \textbf{0.738 adaptive-gate F1 across 7 unseen packages} (cross-project), and 0.715 F1 on the original cross-project set, leading the released SYMGEN~\cite{symgen} checkpoint (0.45), an end-to-end SYMGEN+LoRA reproduction (0.66), BLens~\cite{blens} (0.46 reported), and a StarCoder-3B zero-shot baseline (0.65) on the cross-project set. In summary, we make the following contributions:
...
\begin{itemize}
    ...
    \item We demonstrate that a compact, structure-aware encoder can match or outperform billion-parameter LLM baselines: \sysname{}'s 25M parameters are over 1{,}000$\times$ fewer than SYMGEN's CodeLlama-34B backbone, and it serves predictions in single-digit milliseconds per function, an order of magnitude faster than autoregressive 34B-LLM decoding, making \sysname{} practical for interactive reverse-engineering.
...
\begin{table}[t]
    \centering
    \caption{Main results on the held-out test set and the cross-project evaluation (seven fully-held-out packages, 13{,}581 fns). Adaptive gate routes binaries by external-call Jaccard.}
    \label{tab:main}
    \footnotesize
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        \midrule
        Decoder (beam) & 70.6\% & 0.770 & 0.665 & 56.5\% \\
        $k$-NN+BinFilter & 70.8\% & 0.770 & 0.558 & 44.7\% \\
        Adaptive gate & \textbf{71.0\%} & \textbf{0.777} & \textbf{0.738} & \textbf{58.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Per-package cross-project. Upper: original cross-project set. Lower: . $J_{\max}$ is max external-call Jaccard to training. Gate routes $J_{\max}\!\ge\!0.7$ to $k$-NN, else decoder.}
    \label{tab:perpackage}
    \footnotesize
    \begin{tabular}{lrcccc}
        \toprule
        \textbf{Pkg} & \textbf{Funcs} & \boldmath$J_{\max}$ & \textbf{Dec} & \textbf{$k$-NN+P2} & \textbf{Gate} \\
        \midrule
        ...
        \midrule
        original cross-project set agg. & \textbf{10{,}467} & --- & 0.668 & \textbf{0.715} & 0.714 \\
        cross-project agg. & \textbf{13{,}581} & --- & 0.665 & 0.558 & \textbf{0.738} \\
        \bottomrule
    \end{tabular}
\end{table}

Table~\ref{tab:perpackage} shows the complementary regimes: high-Jaccard packages (nginx118, angie) are dominated by $k$-NN (+0.07--0.17 F1 over decoder) because a near-clone training binary carries the exact name; low-Jaccard packages (dash, gettext, psmisc) are dominated by the LM-pretrained decoder (+0.58--0.77 F1 over $k$-NN) because retrieval returns structurally-similar but wrong neighbors while the decoder composes plausible names from known sub-tokens. The adaptive gate captures both regimes, raising the clean-7 aggregate F1 from 0.558 (pure $k$-NN+BinFilter) to 0.738 without ground-truth access. Recutils stays at 0.35 F1 because its \texttt{rec\_*}/\texttt{recutl\_*} sub-tokens are sparse in both the training corpus and the 397K LM-pretrain corpus, leaving neither head a foothold.
...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        ...
        BLens (retrained on our 278K)~\cite{blens} & USENIX Sec 2025 & ${\sim}200$M & 0.453 \\
...
\begin{table}[t]
    ...
    \begin{tabular}{lrr}
        ...
        \midrule
        \textbf{\sysname{} (ours)} & \textbf{25M} & \textbf{\textasciitilde{}5 ms} \\
...
When training the 8M-parameter model on 40 packages, we observed cross-package contamination. Scaling to 25M parameters resolved this; every cross-project package improved. Our final 300K-trained model uses 25M parameters.
...
We presented \sysname{}, a 25M-parameter GNN+retrieval system that reaches 0.770 F1 on a held-out test set, \textbf{0.738 adaptive-gate F1 across 7 completely unseen packages} (cross-project), and 0.715 F1 on the original cross-project set, beating released SYMGEN (0.45), SYMGEN+LoRA (0.66), BLens (0.46), and StarCoder-3B zero-shot (0.65) on the cross-project set while being over 1{,}000$\times$ smaller than SYMGEN's backbone. Our contributions:
