...
Table~\ref{tab:main} shows $k$-NN with a binary-fingerprint filter outperforms the decoder on cross-project (+2.4pp EM, +0.045 F1) with zero retraining. The filter keeps only retrieval candidates whose parent training binary shares at least Jaccard $\tau{=}0.5$ of external-call vocabulary with the query binary, eliminating cross-package name contamination.
...
\begin{table}[t]
    ...
    \begin{tabular}{llcc}
        ...
        SymLM (BLens repro)~\cite{symlm,blens} & CCS 2022 & --- & 0.277$^\ddagger$ \\
