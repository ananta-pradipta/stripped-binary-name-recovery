...
## Staged fairness runs (user-gated; nothing launched) — 2026-08-27
...
2. BLens on OUR train tier: `python3 scripts/prep_blens_ours_v2.py` → embedding chain (blens_v2 pattern: ghidra_array.sbatch → encode.sbatch for CLAP + PalmTree over dh2/blens_ours_v2/bins.txt, ~1.5–2 d) → `RunExp.py --cross-project -data-dir dh2/blens_ours_v2 -d xp/ours-v2 -pretrain -train -inferBest` in the BLens conda env (~10 h).
